...
- Keep the former `https://skills-hub.hkooii.com/` hostname as a legacy compatibility surface when it remains attached, but mark it `noindex`; canonical metadata, sitemap, documentation, and production acceptance must point at `skills-hub.lapplax.com`.
- Let the production workflow idempotently associate the canonical Pages custom domain through the Cloudflare Pages API and require that domain to reach `active` status before canonical-domain acceptance succeeds.
